...
CREATE INDEX IF NOT EXISTS "Appointment_tenantId_patientId_startsAt_idx" ON "Appointment"("tenantId", "patientId", "startsAt");

-- ClinicalSession: (tenantId, patientId, sessionDate) for patient session queries
CREATE INDEX IF NOT EXISTS "ClinicalSession_tenantId_patientId_sessionDate_idx" ON "ClinicalSession"("tenantId", "patientId", "sessionDate");

-- ReminderLog: (tenantId, createdAt) for audit and purge queries
CREATE INDEX IF NOT EXISTS "ReminderLog_tenantId_sentAt_idx" ON "ReminderLog"("tenantId", "sentAt");

-- PaymentReminderLog: (tenantId, createdAt) for audit and retention queries
CREATE INDEX IF NOT EXISTS "PaymentReminderLog_tenantId_sentAt_idx" ON "PaymentReminderLog"("tenantId", "sentAt");
